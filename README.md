...
## Pushing each repo to its own remote

Because each sub-directory is a self-contained repo, initialize and push
them one at a time. Example for `infrastructure/`:

```bash
cd infrastructure
git init -b main
git add .
git commit -m "initial: infrastructure (compose + nginx + db/clickhouse init)"
git remote add origin git@github.com:YOUR-ORG/url-shortener-infrastructure.git
git push -u origin main
```

Repeat for the four app repos:

```bash
cd ../redirect-service
git init -b main && git add . && git commit -m "initial: go redirect service"
git remote add origin git@github.com:YOUR-ORG/url-shortener-redirect.git
git push -u origin main

cd ../api-service
git init -b main && git add . && git commit -m "initial: fastapi admin API"
git remote add origin git@github.com:YOUR-ORG/url-shortener-api.git
git push -u origin main

cd ../analytics-worker
git init -b main && git add . && git commit -m "initial: analytics worker"
git remote add origin git@github.com:YOUR-ORG/url-shortener-analytics.git
git push -u origin main

cd ../admin-panel
git init -b main && git add . && git commit -m "initial: angular 19 admin panel"
git remote add origin git@github.com:YOUR-ORG/url-shortener-admin.git
git push -u origin main
```

Each repo has its own `.gitignore`, `.dockerignore`, `LICENSE` (MIT),
`.env.example`, `Dockerfile`, `docker-compose.yml`, and `Makefile`.

> **Do NOT** run `git init` in this root directory — the top-level is
> an orchestration scratchpad, not a repo.
